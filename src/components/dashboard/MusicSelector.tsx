  ...
  return (
    <div className="space-y-6">
        ...
        {PRESET_MUSIC.map((music) => (
          ...
          >
            ...
            <div className="overflow-hidden w-full">
              <p className="text-[10px] font-bold text-gray-800 line-clamp-1">{music.name}</p>
              <p className="text-[8px] text-gray-400 uppercase tracking-widest">{music.artist}</p>
            </div>
            {selectedMusic === music.url && (
              <div className="absolute top-1 right-1 w-3 h-3 bg-wedding-gold rounded-full flex items-center justify-center text-white shadow-sm">
      ...
      <div className="relative pt-4">
        ...
        <div className="relative flex justify-center text-[10px] uppercase font-black tracking-widest text-gray-400 bg-white px-4">Opsi Canggih</div>
      </div>

      <div className="flex flex-col items-center gap-6 py-6 bg-gray-50/50 rounded-3xl border border-gray-100">
        {/* Tombol Generator */}
        ...
        {isCustomSelected && (
          <div 
            className={`w-full max-w-xs p-4 rounded-2xl border-2 bg-white shadow-xl flex items-center justify-between animate-in fade-in zoom-in duration-500 ${
            ...
            <div className="flex items-center gap-4">
              <div 
                ...
                className={`w-12 h-12 rounded-full flex items-center justify-center transition-all shadow-md cursor-pointer ${
                  playing === selectedMusic ? 'bg-wedding-gold text-white rotate-12 scale-110' : 'bg-wedding-gold/10 text-wedding-gold hover:bg-wedding-gold/20'
                }`}
              >
              ...
              <div>
                ...
                <p className={`text-[9px] font-bold uppercase tracking-widest ${isAiGenerated ? 'text-wedding-gold animate-pulse' : 'text-wedding-sage'}`}>
                  {isAiGenerated ? "Murni Gubahan AI" : "Kurasi Suasana AI"}
                </p>
              </div>
            </div>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white shadow-lg ${isAiGenerated ? 'bg-wedding-gold' : 'bg-wedding-sage'}`}>
        ...
        {aiMessage && (
          <div className="px-6 text-center">
            <p className="text-[9px] text-wedding-text/60 leading-relaxed max-w-[200px] mx-auto italic">
              "{aiMessage}"
            </p>
          </div>
        )}
        ...
        <div className="w-full max-w-xs px-4">
          <div className="relative flex justify-center text-[9px] uppercase font-black tracking-widest text-gray-300 mb-4 italic">Atau Upload MP3 Sendiri</div>
          <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
            <input 
              ...
              className="w-full text-[9px] text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-[9px] file:font-black file:uppercase file:bg-gray-100 file:text-gray-600 hover:file:bg-gray-200 cursor-pointer" 
            />
          </div>
          <div className="flex items-start gap-2 mt-4 text-[9px] text-gray-400 italic">
