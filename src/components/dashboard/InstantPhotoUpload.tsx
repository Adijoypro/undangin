...
export default function InstantPhotoUpload({ 
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`flex flex-wrap items-center gap-4 p-4 rounded-2xl border border-dashed ${currentTheme.border} bg-wedding-text/[0.03] transition-colors duration-500`}>
        {/* Preview Square */}
        <div className="w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0 bg-wedding-text/[0.05] rounded-xl border border-wedding-gold/10 flex items-center justify-center overflow-hidden shadow-inner relative transition-colors duration-500">
          {photoUrl ? (
            <img src={photoUrl} className="w-full h-full object-cover" alt="Preview" />
          ) : (
            <div className="text-center p-2">
              <svg className="w-6 h-6 text-wedding-text/20 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <p className="text-[7px] text-wedding-text/40 font-bold uppercase">No Photo</p>
            </div>
          )}
          {uploading && (
        ...
        {/* Upload Actions */}
        <div className="flex-1 min-w-[140px] flex flex-col justify-center gap-2">
          <input 
            type="file" 
            accept="image/*,.heic,.heif" 
            onChange={handleFileChange}
            className={`w-full text-[9px] text-wedding-text/40 font-medium file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[10px] file:font-bold ${currentTheme.bg} ${currentTheme.text} ${currentTheme.hover} cursor-pointer transition-all active:scale-95`} 
          />
